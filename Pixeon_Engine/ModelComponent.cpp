...
bool AdvancedModelComponent::LoadModel(const std::string& assetName, bool force) {
    ...
    if (!m_resource->LoadFromAsset(assetName)) {
        ShowErrorMessage("Model Load Error", "[LoadModel] Asset �ǂݍ��ݎ��s: " + assetName);
        m_resource.reset();
        return false;
    }
    if (!m_resource->BuildGPU(DirectX11::GetInstance()->GetDevice())) {
        ShowErrorMessage("Model GPU Error", "[LoadModel] GPU �o�b�t�@�쐬���s: " + assetName);
        m_resource.reset();
        return false;
    }
    ...
    if (!m_resource->animations.empty()) {
        if (!m_animCtrl.Play(0, true, 1.0)) {
            ShowErrorMessage("Animation Error", "[LoadModel] �ŏ��̃A�j���[�V�����Đ��Ɏ��s");
        }
        else {
            m_selectedClip = 0;
        }
    ...
    if (!ResolveAndLoadMaterialTextures()) {
        ShowErrorMessage("Material Load Warning", "[LoadModel] �}�e���A���e�N�X�`�������̈ꕔ�Ɏ��s");
    }
    ClassifySubMeshes();
    m_loaded = true;
...
void AdvancedModelComponent::EnsureInputLayout(const std::string& vs) {
    ...
    if (!ShaderManager::GetInstance()->GetVSBytecode(vs, &bc, &sz)) {
        ShowErrorMessage("Shader Error", "[EnsureInputLayout] VS Bytecode ���擾: " + vs);
        return;
    }
...
bool AdvancedModelComponent::PlayAnimation(const std::string& clipName, bool loop, double speed) {
    ...
    for (int i = 0; i < (int)m_resource->animations.size(); i++) {
        if (m_resource->animations[i].name == clipName) {
            if (!m_animCtrl.Play(i, loop, speed)) {
                ShowErrorMessage("Animation Error", "[PlayAnimation] �Đ��Ɏ��s: " + clipName);
                return false;
            }
            m_selectedClip = i;
            return true;
    ...
    ShowErrorMessage("Animation Error", "[PlayAnimation] �N���b�v�����݂��܂���: " + clipName);
    return false;
}
    ...
    for (int i = 0; i < (int)m_resource->animations.size(); i++) {
        if (m_resource->animations[i].name == clipName) {
            if (!m_animCtrl.BlendTo(i, duration, mode, loop, speed)) {
                ShowErrorMessage("Animation Error", "[BlendToAnimation] �u�����h�J�n���s: " + clipName);
                return false;
            }
            return true;
        }
    }
    ShowErrorMessage("Animation Error", "[BlendToAnimation] �N���b�v�����݂��܂���: " + clipName);
    return false;
}
...
bool AdvancedModelComponent::SetMaterialTexture(uint32_t matIdx, uint32_t slot, const std::string& assetName) {
    ...
    if (matIdx >= m_resource->materials.size()) {
        ShowErrorMessage("Material Error", "[SetMaterialTexture] material index �͈͊O");
        return false;
    }
    ...
    if (!AssetsManager::GetInstance()->LoadAsset(assetName, data)) {
        ShowErrorMessage("Material Error", "[SetMaterialTexture] �e�N�X�`�����Y�擾���s: " + assetName);
        return false;
    }
    ...
    if (FAILED(hr)) {
        ShowErrorMessage("Material Error", "[SetMaterialTexture] �f�R�[�h���s: " + assetName);
        return false;
    }
...
void AdvancedModelComponent::RefreshCachedModelAssetList(bool force) {
    ...
    // ������Ȃ������ꍇ�͏]���C���f�b�N�X���ێ��i���݂��Ȃ��Ȃ玩�R�ɊO���j
    if (newIndex != -1) {
...
void AdvancedModelComponent::DrawSection_Model() {
    ...
    if (m_cachedModelAssets.empty()) {
        ...
        ImGui::TextUnformatted("�L���b�V���ς݃��f���Ȃ� / AssetsManager::LoadAsset �œǂݍ���ł�������");
    }
    else {
            ...
            for (int i = 0; i < (int)m_cachedModelAssets.size(); ++i) {
                ...
                if (ImGui::Selectable(m_cachedModelAssets[i].c_str(), sel)) {
                    ...
                    if (i >= 0 && i < (int)m_cachedModelAssets.size())
                        m_selectedCachedAssetName = m_cachedModelAssets[i]; // �I��ێ�������X�V
                }
        ...
        if (ImGui::Button("Load Selected") && canLoad) {
            if (!LoadModel(m_cachedModelAssets[m_cachedAssetSelected], true)) {
                ShowErrorMessage("Model Load Error", "�I�����f���̃��[�h�Ɏ��s���܂����B");
            }
        }
    ...
    if (ImGui::Button("Load Path")) {
        if (m_modelPathInput[0]) {
            if (!LoadModel(m_modelPathInput, true)) {
                ShowErrorMessage("Model Load Error", "�蓮�w�胍�[�h���s: " + std::string(m_modelPathInput));
            }
        }
    }
    ImGui::SameLine();
    if (ImGui::Button("Reload")) {
        if (!m_currentAsset.empty()) {
            if (!LoadModel(m_currentAsset, true)) {
                ShowErrorMessage("Model Reload Error", "�����[�h���s: " + m_currentAsset);
            }
        }
    }
            ...
            if (ImGui::Button("LoadInput")) {
                if (input[0]) {
                    if (!SetMaterialTexture((uint32_t)m_selectedMaterial, (uint32_t)i, input)) {
                        ShowErrorMessage("Material Error", "�e�N�X�`���ǂݍ��ݎ��s: " + std::string(input));
                    }
                }
            }
            ImGui::SameLine();
